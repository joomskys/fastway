 ...
 * Post Thumbnail
*/
if(!function_exists('fastway_post_thumbnail')){
    function fastway_post_thumbnail($args=[]){
        $args = wp_parse_args($args,[
            'id'              => null,
            'thumbnail_size'  => is_single() ? 'large' : 'medium',
            'echo'            => true,
            'default_thumb'   => fastway_configs('fastway_default_post_thumbnail'),
            'thumbnail_is_bg' => fastway_configs('fastway_thumbnail_is_bg'),
            'img_class'       => '',
            'show_image'      => true    
        ]);
        extract($args);
        ...
        $thumbnail_atts = [];
        // class
        $thumbnail_atts_class = ['post-image','cms-post',$args['img_class']];
        ...
        if($thumbnail_is_bg) $thumbnail_atts_style[] = 'background-image: url('.fastway_get_image_url_by_size(['id'=>$id,'size'=> 'full', 'default_thumb' => $default_thumb]).')';
        ...
        ?>
            ...
            return '<div '.implode(' ', $thumbnail_atts).'><img src="'.fastway_get_image_url_by_size(['id'=>$id,'size'=> 'full', 'default_thumb' => $default_thumb]).'" alt="'.get_the_title().'" class="'.$args['img_class'].'" /></div>'.do_action('fastway_post_thumbnail_content');
        }
    }
}
 ...
 * Post Gallery 
*/
if(!function_exists('fastway_post_gallery')){
    function fastway_post_gallery($args=[]){
        $args = wp_parse_args($args, array(
            'id'             => null,
            'show_media'     => '1',
            'thumbnail_size' => 'large',
            'show_author'    => is_singular() ? fastway_get_opts('archive_author_on','1') : fastway_get_opts('post_author_on','1'),
            'echo'           => true,
            'default_thumb'  => apply_filters('fastway_default_post_thumbnail', false),
        ...
        $gallery_list = explode(',', fastway_get_post_format_value($args['id'], $args['source'], ''));
                ...
                <div class="cms-post-gallery-slide cms-slick-slide <?php if($light_box) {echo 'images-light-box';} ?>" >
	                ...
	                foreach ($gallery_list as $img_id):
	                    ...
	                    <div class="carousel-item slick-slide-item">
	                        <a class="light-box" href="<?php echo esc_url(wp_get_attachment_image_url($img_id, 'full'));?>"><img src="<?php echo fastway_get_image_url_by_size(['id' => $img_id, 'size' => $args['thumbnail_size']]);?>" alt="<?php echo esc_attr(get_post_meta( $img_id, '_wp_attachment_image_alt', true )) ?>"></a>
	                    </div>
	                    <?php
	                endforeach;
	                ?>
 ...
 * Post Video
*/
if(!function_exists('fastway_post_video')){
    function fastway_post_video($args=[]){
        global $wp_embed;
        $args = wp_parse_args($args, [
        ...
        $video_url = fastway_get_post_format_value($args['id'], $args['video_url'], '');
        $video_file = fastway_get_post_format_value($args['id'], $args['video_file'], []);
            $video_file_id  = isset($video_file['id']) ? $video_file['id'] : '';
        $video_html = fastway_get_post_format_value($args['id'], $args['video_html'], '');
        ...
        if (!empty($video_url)) {
        	...
        	$video = fastway_post_thumbnail($args);
        }
        $video .= ob_get_clean();
        // Show video 
        if($args['echo'])
            echo '<div class="entry-video">'.apply_filters('fastway_post_video', $video).'</div>';
        else 
 ...
 * Post Audio
*/
if(!function_exists('fastway_post_audio')){
    function fastway_post_audio($args = []){
        $args = wp_parse_args($args, [
        ...
        global $wp_embed;
        $audio_url = fastway_get_post_format_value($args['id'], $args['audio_url'], '');
        $audio_file = fastway_get_post_format_value($args['id'], $args['audio_file'], ['id'=>'']);
        ...
        if(!empty($audio_url)){
            ...
            $audio = fastway_post_thumbnail($args);
        }
        $audio .= ob_get_clean();
        // Show video 
        if($args['echo'])
            echo apply_filters('fastway_post_audio', $audio);
        else 
            return $audio;
    }
}
 ...
 * Post Quote
*/
if(!function_exists('fastway_post_quote')){
    function fastway_post_quote($args = []){
        $args = wp_parse_args($args, [
            'id'             => null,
            'thumbnail_size' => is_single() ? 'large' : 'medium',
            'echo'           => true,
        ...
        $text = fastway_get_post_format_value($args['id'], 'post-quote-text', '');
        $cite = fastway_get_post_format_value($args['id'], 'post-quote-cite', '');
        $quote = '';
        $quote_attrs = $quote_style = [];
        $quote_css_class = ['quote-wrap'];
        
        // Inline Style
        $bg_img = fastway_get_image_url_by_size([
            'id'   => $args['id'],
            'size' => 'post-thumbnail'
        ]);
        if(!empty($bg_img)) {
        ...
        if(!empty($text) || !empty($cite)){
            ...
            $quote = fastway_post_thumbnail($args);
        }

        if($args['echo'])
            echo apply_filters('fastway_post_quote', $quote);
        else 
            return $quote;
    }
}
 ...
 * Post Link
*/
if(!function_exists('fastway_post_link')){
    function fastway_post_link($args = []){
        $args = wp_parse_args($args, [
            'id'             => null,
            'thumbnail_size' => is_single() ? 'large' : 'medium',
            'echo'           => true,
        ...
        $title = fastway_get_post_format_value($args['id'], 'post-link-title', esc_html__('View Our Portfolio','fastway'));
        //https://themeforest.net/user/zookastudio/portfolio
        $link = fastway_get_post_format_value($args['id'], 'post-link-url', '');
        if(empty($link)) return;
        // Get first link in content 
        $link_in_content =  fastway_get_content_link(['echo' => false]);
        // Link attribute
        ...
        if(!empty($link)) {
            ...
            // link 
            $link = '<div '.trim(implode(' ', $link_attrs)).'>' . fastway_get_content_link(['echo' => false]) . '</div>';
        } else {
            $link = fastway_post_thumbnail($args);
        }

        if($args['echo'])
            echo apply_filters('fastway_post_link', $link);
        else 
            return $link;
    }
}
 ...
 * @since 1.0.1
*/
if(!function_exists('fastway_post_image')){
    function fastway_post_image($args = []){
        $args = wp_parse_args($args, [
            'id'             => null,
            'thumbnail_size' => is_single() ? 'large' : 'medium',
            'echo'           => true,
        ...
        // Get first link in content 
        $image_in_content =  fastway_get_content_image(['echo' => false]);
        
        if(has_post_thumbnail($args['id'])){
           $image =  fastway_post_thumbnail($args);
        } elseif(!empty($image_in_content) && !is_single()){
            // images
            $image =  fastway_get_content_image(['echo' => false]);
        }
        if($args['echo'])
            echo apply_filters('fastway_post_image', $image);
        else 
            return $image;
    }
}
 ...
 * @since 1.0.1
*/
if(!function_exists('fastway_post_media')){
    function fastway_post_media($args = []){
        $args = wp_parse_args($args, [
            'id'             => null,
            'thumbnail_size' => is_single() ? 'large' : 'medium',
            'echo'           => true,
        ...
        do_action('fastway_before_post_media');
    ...
    <div class="<?php echo trim(implode(' ', $classes));?>"><div class="cms-featured-inner relative"><?php
        printf('%s', $args['before']);
            switch (get_post_format($args['id'])) {
                case 'gallery':
                    fastway_post_gallery($args);
                    break;
                case 'video':
                    fastway_post_video($args);
                    break;
                case 'audio':
                    fastway_post_audio($args);
                    break;
                case 'quote':
                    fastway_post_quote($args);
                    break;
                case 'link':
                    fastway_post_link($args);
                    break;
                 case 'image':
                    fastway_post_image($args);
                    break;
                default:
                    fastway_post_thumbnail($args);
                    break;
        ...
        printf('%s', $args['after']);
        do_action('fastway_post_media_content', $args);
    ?></div></div>
    <?php
        do_action('fastway_after_post_media');
    }
}
if(!function_exists('fastway_loop_media')){
    function fastway_loop_media($args = []){
        $args = wp_parse_args($args, [
            'show_media'     => '1',
            'thumbnail_size' => 'large',
            'show_author'    => '1'
        ...
        fastway_post_media($args); 
    }
}

/**
 * Post Author's on Media 
 * action hook : fastway_post_thumbnail_content
 * @since 1.0.0
*/
if(!function_exists('fastway_post_author_on_media')){
    function fastway_post_author_on_media($args = []){
        $args = wp_parse_args($args, [
			'echo'        => true,
			'show_author' => is_singular() ? fastway_get_opts('post_author_on','1') : fastway_get_opts('archive_author_on','1')
        ]);
        extract($args);
 ...
 * action hook: fastway_post_thumbnail_content
 * @since 1.0.0
 * add_action('fastway_post_thumbnail_content', 'fastway_post_category_on_media', 10);
*/
if(!function_exists('fastway_post_category_on_media')){
    function fastway_post_category_on_media($args =[]){
        $args = wp_parse_args($args, [
            'echo'     => true,
            'show_cat' => is_singular() ? fastway_get_opts('post_categories_on','1') : fastway_get_opts('archive_categories_on','1'),
            'taxonomy' => 'category',
 ...
 *
*/
if(!function_exists('fastway_post_readmore_on_media')){
    function fastway_post_readmore_on_media($args=[]){
        $args = wp_parse_args($args, [
            'icon' => 'fa fa-plus',
