 ...
 * @return mixed
 */
function fastway_get_opt( $opt_id, $default = false ) {
 ...
 * @return bool|mixed
 */
function fastway_get_post_format_value_old( $post_format_key, $default = false ) {
	global $post;
 ...
 * @return string
 */
function fastway_get_opt_name() {
	return apply_filters( 'fastway_opt_name', 'cms_theme_options' );
}
 ...
 * @return string
 */
function fastway_get_post_opt_name() {
	return apply_filters( 'fastway_post_opt_name', 'fastway_post_options' );
}
 ...
 */
function fastway_get_page_titles() {
	$title = '';
	...
	if ( ! is_archive() ) {
		...
		$title = esc_html__( 'Author:', 'fastway' ) . ' ' . get_the_author();
	} // Author
	else {
		$title = get_the_archive_title();
		if( (class_exists( 'WooCommerce' ) && is_shop()) ) {
			$title = esc_html__( 'Our Products', 'fastway' );
		}
	}
 ...
 */
function fastway_get_the_excerpt( $length = 55, $post = null ) {
	$post = get_post( $post );
	...
	if ( post_password_required( $post ) ) {
		return esc_html__( 'Post password required.', 'fastway' );
	}

	$content = apply_filters( 'the_content', strip_shortcodes( $post->post_content ) );
	$content = str_replace( ']]>', ']]&gt;', $content );

	$excerpt_more = apply_filters( 'fastway_excerpt_more', '&hellip;' );
 ...
 * @return boolean
 */
function fastway_is_valid_color( $color ) {
 ...
 * @return string
 */
function fastway_css_minifier( $css ) {
 ...
 */
function fastway_header_code() {
	$site_header_code = fastway_get_opt( 'site_header_code' );
	if ( $site_header_code !== '' ) {
		print wp_kses( $site_header_code, wp_kses_allowed_html() );
	}
}

add_action( 'wp_head', 'fastway_header_code' );

/**
 * Footer Tracking Code to wp_footer hook.
 */
function fastway_footer_code() {
	$site_footer_code = fastway_get_opt( 'site_footer_code' );
	if ( $site_footer_code !== '' ) {
...
function fastway_add_subtitle_field() {
	...
	if ( in_array( $screen->id, array( 'acm-post' ) ) ) {
		...
		echo '<div class="subtitle"><input type="text" name="post_subtitle" value="' . esc_attr( $value ) . '" id="subtitle" placeholder = "' . esc_html__( 'Subtitle', 'fastway' ) . '" style="width: 100%;margin-top: 4px;"></div>';
	}
}

add_action( 'edit_form_after_title', 'fastway_add_subtitle_field' );

/**
 * Save custom theme meta
 */
function fastway_save_meta_boxes( $post_id ) {
...
add_filter( 'enable_cms_carousel', 'fastway_enable_cms_carousel' );
function fastway_enable_cms_carousel() {
	return false;
}
...
if(!function_exists('fastway_custom_section_classes')){
    ...
    function fastway_custom_section_classes($classes, $settings){
        if(isset($settings['column_hori_align']) && !empty($settings['column_hori_align'])){
            $classes[] = 'style-' . $settings['column_hori_align'];
        }
        return $classes;
    }
}