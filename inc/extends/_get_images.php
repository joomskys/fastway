 ...
 */
function fastway_get_image_sizes() {
    global $_wp_additional_image_sizes;
    $sizes = array();
 ...
 *
 * @uses   fastway_get_image_sizes()
 * @param  string $size The image size for which to retrieve data.
 * @return bool|array $size Size data about an image size or false if the size doesn't exist.
 */
function fastway_get_image_size( $size ) {
    $sizes = fastway_get_image_sizes();
 ...
 *
 * @uses   fastway_get_image_size()
 * @param  string $size The image size for which to retrieve data.
 * @return bool|string $size Width of an image size or false if the size doesn't exist.
 */
function fastway_get_image_width( $size ) {
    if ( ! $size = fastway_get_image_size( $size ) ) {
        return false;
    }
    if ( isset( $size['width'] ) ) {
        return $size['width'];
 ...
 */
function fastway_get_image_height( $size ) {
    if ( ! $size = fastway_get_image_size( $size ) ) {
        return false;
    }
...
*/
if (!function_exists('fastway_default_image_thumbnail_url')) {
    function fastway_default_image_thumbnail_url($args = array())
    {
        ...
        extract($args);
        /* use fastway_resize_thumbnail( $attach_id = null, $img_url = null, $width, $height, $crop = false ) */
        global $_wp_additional_image_sizes;
        $image_sizes = fastway_get_image_sizes();
        $size = explode('x', $size);
        $size_use = $size[0];
        ...
        $default_img = fastway_resize_thumbnail('', $default_img, $width, $height, true);
...
*/
if (!function_exists('fastway_default_image_thumbnail')) {
    function fastway_default_image_thumbnail($args = array())
    {
        $args = wp_parse_args($args, array(
            'size'        => 'large',
            'default_img' => '/wp-content/themes/' . get_template() . '/assets/images/placeholder/no-image.jpg',
...
* 
*/
if ( ! function_exists( 'fastway_resize_thumbnail' ) ) {
    /**
     * @param int $attach_id
     ...
     * @return array
     */
    function fastway_resize_thumbnail( $attach_id = null, $img_url = null, $width, $height, $crop = false ) {
    ...
    function fastway_image_by_size( $args = []) {
        ...
        if(empty($id) ){
            ...
            if ( is_array( $size ) ) {
                ...
                $p_img = fastway_resize_thumbnail( $id, null, $size[0], $size[1], true );
                ...
                $attributes = fastway_stringify_attributes( $img_atts );
                if($show_image){
                    $fastway_image_by_size = '<img ' . $attributes . ' />';
                } else {
                    $fastway_image_by_size = '<div style="width:'.$p_img['width'].'px;height:'.$p_img['height'].'px;max-width:100%;"></div>';
                }
            }
        }

        if($echo)
            echo fastway_html($args['before'].$fastway_image_by_size.$args['after']);
        else 
            return $args['before'].$fastway_image_by_size.$args['after'];
    }
}

if(!function_exists('fastway_get_image_url_by_size')){
    function fastway_get_image_url_by_size($args = []) {
        ...
        if(empty($id) && $default_thumb){
            $img_url = fastway_default_image_thumbnail_url(['size' => $size, 'class' => $args['class']]);
        } elseif ( is_string( $size ) && ( ( ! empty( $_wp_additional_image_sizes[ $size ] ) && is_array( $_wp_additional_image_sizes[ $size ] ) ) || in_array( $size, array(
                    'thumbnail',
                    'thumb',
                    'medium',
 ...
 * @return string
 */
function fastway_stringify_attributes( $attributes ) {
    $atts = array();
 ...
 *
*/
if(!function_exists('fastway_image_dimensions')){
    function fastway_image_dimensions( $id = null , $size = 'medium', $dimensions = 'height', $echo = false ) {
        global $_wp_additional_image_sizes;
