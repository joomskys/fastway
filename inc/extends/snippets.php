...
**/
if(!function_exists('fastway_content')){
    function fastway_content($id){
...
**/
if(!function_exists('fastway_content_by_slug')){
    function fastway_content_by_slug($slug, $post_type){
        $content = fastway_get_content_by_slug($slug, $post_type);
        $id = fastway_get_id_by_slug($slug, $post_type);
        $shortcodes_custom_css = get_post_meta( $id, '_wpb_shortcodes_custom_css', true );
        if ( ! empty( $shortcodes_custom_css ) ) {
            $shortcodes_custom_css = strip_tags( $shortcodes_custom_css );
 ...
 *
*/
function fastway_get_content_link( $args = []){
 ...
 *
*/
function fastway_get_content_image( $args = []){
    $args = wp_parse_args($args, [
        'content' => '',
        'class'   => 'content-image',
        'echo'    => true
    ...
    if( preg_match( '/<img\s[^>]*?srcset=([\'"])(.+?)\1/is', $args['content'], $_srcset )) { 
        ...
        $img_id = fastway_get_attachment_id_from_url($src);
 ...
 */
if ( ! function_exists( 'fastway_get_attachment_id_from_url' ) ) {
    
    function fastway_get_attachment_id_from_url( $url ) {
 ...
 * 
*/
function fastway_extract_numbers($string,$pos=0)
{
 ...
 * @return ID
*/
function fastway_get_id_by_title($post_title, $post_type = 'page'){
 ...
 * Output html
*/
if(!function_exists('fastway_html')){
    function fastway_html($html){
        return $html;
    }
}
 ...
 * @since 1.0.0
*/
if(!function_exists('fastway_get_custom_post_taxonomies')){
    function fastway_get_custom_post_taxonomies($post_type, $key)
    {
 ...
 * @since 1.0.0
*/
if(!function_exists('fastway_get_custom_post_cat_taxonomy')){
    function fastway_get_custom_post_cat_taxonomy()
    {
        $post = get_post();
 ...
 * @since 1.0.0
*/
if(!function_exists('fastway_get_custom_post_tag_taxonomy')){
    function fastway_get_custom_post_tag_taxonomy()
    {
        $post = get_post();
        $tax_names = get_object_taxonomies($post);
        $result = 'post_tag';
...
*/
function fastway_get_taxo_slug_as_css_class($args = [])
{
...
function fastway_terms($args=[]){
    ...
    echo apply_filters('fastway_terms', $terms_string);
}
 ...
 *
*/
function fastway_get_term_id_by_slug($post_type, $taxo_key, $term_slugs){
    if(empty($term_slugs)) return;
    $term_slugs = explode(',', $term_slugs);
    $term_ids = [];
    foreach ($term_slugs as $slug) {
        $term = get_term_by('slug', $slug, fastway_get_custom_post_taxonomies( $post_type , $taxo_key));
 ...
 *
*/
function fastway_tax_query($post_type, $taxonomies, $taxonomies_exclude ){
    $tax_query = array();    
    if(!empty($taxonomies) || !empty($taxonomies_exclude)) {
        ...
        foreach ($terms as $term) {
            $real_terms_args = [
                'taxonomy' => fastway_get_custom_post_taxonomies( $post_type , $term), 
                'exclude'  => fastway_get_term_id_by_slug($post_type, $term, $taxonomies_exclude)
            ];
...
function fastway_hex2rgba($color, $opacity = false) {
